...
void Chat::createUser()
{
    ...
    std::string name;
    std::cin >> name;
    pair<string, string> logpass(login, password);
...
//         std::shared_ptr<User> user (new User(login, password, name));
         std::shared_ptr<User> user (new User(logpass, name));
...
void Chat::writeMessage()
{
    ...
    while(flag) {
        ...
        if (tmp==72 || tmp==75 || tmp==80 || tmp==77) //повтор для стрелок
              tmp = getch();
        if(((tmp>=32)&&(tmp<=122)) || tmp==13 || tmp==8) {     //диапазон допустимых символов в сообщении
            if((tmp!=64)&&(tmp!=13)) {
            cout << char(tmp);
            text+=char(tmp);
            }
